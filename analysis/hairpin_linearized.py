...
for session in sessions:
    ...
    binned_tracking = data_utils.bin_tracking_data_by_arena_position(body_tracking)


    # ! get when moving towards the goal
    when_heading_to_goal = np.where((derivative(body_tracking['global_coord']) > 0)&(body_tracking['speed'] > .05))[0]
    heading_to_goal = data_utils.select_by_indices(body_tracking, when_heading_to_goal)
    binned_heading_to_goal = data_utils.bin_tracking_data_by_arena_position(heading_to_goal)

    # ! get when moving away the goal
    when_heading_away_goal = np.where((derivative(body_tracking['global_coord']) < 0)&(body_tracking['speed'] > .05))[0]
    heading_away_goal = data_utils.select_by_indices(body_tracking, when_heading_away_goal)
    binned_heading_away_goal = data_utils.bin_tracking_data_by_arena_position(heading_away_goal)
    ...
    plot_tracking_xy(heading_to_goal, 'orientation', ax=axes_dict['F'], vmin=0, vmax=360, cmap='bwr')
    plot_tracking_xy(heading_away_goal, 'orientation', ax=axes_dict['D'], vmin=0, vmax=360, cmap='bwr')
    ...
    axes_dict['C'].axhline(np.nanmean(binned_heading_to_goal['speed']), lw=2, ls=':', zorder=-1)
    ...
    axes_dict['E'].axhline(np.nanmean(binned_heading_away_goal['speed']), lw=2, ls=':', zorder=-1)
    ...
    axes_dict['C'].set(xlabel='arena position', ylabel='running speed to goal cm/s')
    axes_dict['D'].set(ylabel='orient mving away goal')
    axes_dict['E'].set(xlabel='arena position', ylabel='running speed away goal cm/s')
