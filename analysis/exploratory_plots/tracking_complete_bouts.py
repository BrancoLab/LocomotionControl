...
def align_in_space(bouts:pd.DataFrame, tracking:Dict[str, pd.DataFrame]) -> pd.DataFrame:
    '''
        Get the average and standard deviation of speed and angular velocity
        as a function of position in the hairpin track
    '''
    data = dict(speed=[], speed_std=[], angular_velocity_std=[], angular_velocity=[])
    ...
    for variable in ('speed', 'angular_velocity'):
        ...
        for i,bout in bouts.iterrows():
            variable_data = tracking[bout['name']][variable][bout.start_frame:bout.end_frame]
            
            variable_data = data_utils.convolve_with_gaussian(variable_data, 11)
            coord = tracking[bout['name']]['global_coord'][bout.start_frame:bout.end_frame]
            ...
            for n, th in enumerate(bins):
                ...
                aligned[th].extend(variable_data[(coord > pre)& (coord <= post)])

        data[variable] = np.array([np.nanmean(v) for v in aligned.values()])
        data[variable+'_std'] = np.array([np.nanstd(v) for v in aligned.values()])
...
# get bouts
bouts = pd.DataFrame((db_tables.LocomotionBouts & 'complete="true"' & 'direction="outbound"').fetch())
...
for sess in sessions:
    tracking[sess] = db_tables.Tracking.get_session_tracking(sess, body_only=True)
...
for i,bout in bouts.iterrows():
    ...
    axes['B'].plot(coord, speed, color=colors.speed, alpha=.1)
    axes['C'].plot(coord, avel, color=colors.angular_velocity, alpha=.1)

axes['C'].axhline(0, lw=2, ls='--', color=[.3, .3, .3],zorder=-1)

# plot mean and std of velocities
plot_mean_and_error(aligned_tracking.speed, aligned_tracking.speed_std, axes['B'], x=aligned_tracking.global_coord, color='salmon')
plot_mean_and_error(aligned_tracking.angular_velocity, aligned_tracking.angular_velocity_std, axes['C'], x=aligned_tracking.global_coord, color='salmon')

# histogram of bouts durations
axes['D'].hist(bouts.duration, bins=20, color=[.3, .3, .3])
