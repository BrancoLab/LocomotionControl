...
recorder.start(
    base_folder=folder, folder_name="all_tracking", timestamp=False
)
...
for session in track(sessions):
    ...
    tracking = Tracking.get_session_tracking(session, body_only=False, movement=False)
    ...
    for bp, color in zip((snout, paw, tail), (teal, salmon, indigo)):
        draw.Tracking.scatter(bp.x[::30], bp.y[::30], color=color, alpha=.5, zorder=100, ax=ax, label=bp.bpname)
