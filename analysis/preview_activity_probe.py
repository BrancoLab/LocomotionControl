...
for i, recording in recordings.iterrows():
    ...
    colors = [rs.color if rs.brain_region in TARGETS else ('k' if rs.color == 'k' else blue_grey) for i, rs in rsites.iterrows()]
    axes_dict['A'].scatter(
        x,
    ...
    for i in range(len(x)):
        if i %  5 == 0:
            axes_dict['A'].annotate(f'{i} - {rsites.brain_region.iloc[i]}', (x[i]-.3, rsites.probe_coordinates.iloc[i]))
    ...
    colors = [c if r in TARGETS else (c if c == 'k' else blue_grey) for c,r in zip(_colors, _regions)]
    axes_dict['B'].barh(
    ...
    # cleanup and save
    axes_dict['A'].set(ylabel="Probe position (um)", xticks=[], xlim=[0.5, 1.5], ylim=[0, 8000])
