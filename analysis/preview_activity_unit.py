...
TARGETS = ('PRNr', 'PRNc', 'CUN', 'GRN', 'MB', 'PPN', 'RSPagl2/3', 'RSPagl5', 'RSPagl6', 'RSPd1', 'RSPd2', )
...
for i, recording in recordings.iterrows():
    ...
    recorder.start(
        base_folder=base_folder, name=rname, timestamp=False
    )
    ...
    for n, unit in units.iterrows():
        x,y,speed = tracking['x'].values, tracking['y'].values, tracking['speed'].values
        avel, orientation = tracking['angular_velocity'].values, tracking['orientation'].values
        time = np.arange(len(x))
        spike_times = unit.spikes
        spike_times = spike_times[spike_times < len(x)] # ! remove and figure out why necessary
        color = rsites.iloc[unit["site_id"]].color

        # make figure
        f = plt.figure(figsize=(18, 12))
        f.suptitle(f'unit {unit["unit_id"]} - {rsites.iloc[unit["site_id"]].brain_region}')
        ...
        axes_dict['A'].scatter(x[spike_times], y[spike_times], color=color, zorder=100, alpha=.5)

        # plot against speed
        axes_dict['C'].plot(time, speed, color=blue_grey)
        axes_dict['C'].scatter(time[spike_times], speed[spike_times], color=color, zorder=100, alpha=.5)

        # plot against speed
        axes_dict['D'].plot(time, avel, color=blue_grey)
        axes_dict['D'].scatter(time[spike_times], avel[spike_times], color=color, zorder=100, alpha=.5)
