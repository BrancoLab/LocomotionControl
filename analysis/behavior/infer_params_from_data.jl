...
function summary_statistics(x, xname; lowp=0.05, highp=0.95)
...
function get_params_estimate()
    ...
    for rown in pbar(1:size(data, 1))
        ...
        θ = unwrap(atan.(ys .- yt, xs .- xt))[cut:end]  # remove some artifacts
        θ = movingaverage(rad2deg.(θ), 6)

        ω = ∂(time, θ)

        # compute δ and δ̇
        δ = atan.((L .* deg2rad.(ω)), speed)
        δ = movingaverage(δ, 6)
        δ = rad2deg.(δ)
        δ̇ = ∂(time, δ)
        ...
        # plot quantities
        g = g[cut:end]
        plot!(g, x[cut:end]; subplot=1, label=nothing, alpha=0.5, lw=3, color=black)
        plot!(g, y[cut:end]; subplot=2, label=nothing, alpha=0.5, lw=3, color=red)
        plot!(g, u; subplot=3, label=nothing, alpha=0.5, lw=3, color=blue)
        plot!(g, θ; subplot=4, label=nothing, alpha=0.5, lw=3, color=green)
        plot!(g, acceleration; subplot=5, label=nothing, alpha=0.5, lw=3, color=indigo)
        plot!(g, ω; subplot=6, label=nothing, alpha=0.5, lw=3, color=teal)
        plot!(g, δ; subplot=7, label=nothing, alpha=0.5, lw=3, color=salmon)
        plot!(g, δ̇; subplot=8, label=nothing, alpha=0.5, lw=3, color=blue_grey)
    end
