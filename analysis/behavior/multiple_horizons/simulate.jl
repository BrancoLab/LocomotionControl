...
function attempt_step(simtracker, control_model, s0, initial_state, planning_horizon; A=0)
    ...
    for i in 1:3
        _sign = i < 3 ? +1 : -1
        len =
            sqrt(initial_state.v^2 + initial_state.u^2) *
            (planning_horizon + (_sign * 0.01 * i))
        if len < 4
...
function step(simtracker, globalsolution, planning_horizon::Float64; A=5e-5)
    ...
    if !converged(control_model)
        ...
        success, solution = attempt_step(
            simtracker, control_model, s0, initial_state, planning_horizon; A=A
        )
...
function run_simulation(;
    s0=0.0, sf=258, planning_horizon::Float64=0.5, n_iter=1000, Δt=0.01, A=0
)
    ...
    withpbar(pbar) do
        anim = @animate for i in 1:n_iter
            ...
            initial_state, solution, shouldstop, track = step(
                simtracker, globalsolution, planning_horizon; A=A
            )
            ...
            # plot stuff
            draw(:arena)
            draw!(FULLTRACK; border_alpha=0.0, alpha=0.0)

            color = "black"  # colors[i]
            draw!(track; color=color, border_lw=5, alpha=0.0)
            plot_bike_trajectory!(
                solution, bike; showbike=false, label=nothing, color=color, alpha=0.6, lw=4
            )
            draw!(initial_state; color=color, alpha=1)
            plot!(;
                title="T: $(round(simtracker.t; digits=2))s | horizon: $planning_horizon s"
            )
        ...
        if isnothing(simtracker.prevsol)
            ...
            if simtracker.iter > 2
                ...
                mp4(anim, vidpath; fps=(Int ∘ round)(0.25 / Δt))
            end
...
horizons = vcat(collect(0.08:0.01:0.38), collect(0.38:0.02:0.54), collect(0.5:0.05:1.2))
...
for i in 1:length(starts)
    ...
    for horizon in horizons
        ...
        results = run_simulation(;
            planning_horizon=horizon, s0=starts[i], sf=ends[i], A=0.0
        )
    end
end
