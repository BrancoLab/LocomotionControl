...
function compare(; problemtype=:dynamics)
    ...
    bike = Bicycle(; l_f=3, l_r=2, c=4000, width=2.0)
    ...
    # show data
    draw!.(trials; lw=2, alpha=0.15)
    draw!.(cpoints.points)
    plot_bike_trajectory!(solution, bike; showbike=false)

    scatter!(solution.x, solution.y; marker_z=solution.ω, c=:bwr, label=nothing)
    # do comparison
    ...
    # ------------------------------------ fin ----------------------------------- #
    h = histogram(map(t -> t.duration, trials); color="black", label=nothing, xlim=[0, 15])
    duration = solution.t[end]

    plot!(h, [duration, duration], [0, 200]; lw=5, alpha=0.8, color="red", label=nothing)

    l = @layout [
        a{0.5w} grid(5, 1)
    ]
    return display(
        plot(plt, speedplot, uplot, vplot, ωplot, h; layout=l, size=(1600, 1200))
    )
end
