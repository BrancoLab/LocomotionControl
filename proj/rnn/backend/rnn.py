...
class RNN(ABC):
    ...
    def __init__(self, params):
        ...
        if self.load_weights_path is not None:
            ...
            self.initializer = params.get(
                "initializer", WeightInitializer(**params)
            )
        else:
            self.initializer = params.get(
                "initializer", GaussianSpectralRadius(**params)
            )
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            # Input weight matrix:
            self.W_in = tf.compat.v1.get_variable(
                "W_in",
                [N_rec, N_in],
            ...
            self.W_rec = tf.compat.v1.get_variable(
                "W_rec",
                [N_rec, N_rec],
            ...
            self.W_out = tf.compat.v1.get_variable(
                "W_out",
                [N_out, N_rec],
            ...
            # Recurrent bias:
            self.b_rec = tf.compat.v1.get_variable(
                "b_rec",
                [N_rec],
            ...
            # Output bias:
            self.b_out = tf.compat.v1.get_variable(
                "b_out",
                [N_out],
            ...
            self.Dale_rec = tf.compat.v1.get_variable(
                "Dale_rec",
                [N_rec, N_rec],
            ...
            self.Dale_out = tf.compat.v1.get_variable(
                "Dale_out",
                [N_rec, N_rec],
            ...
            self.input_connectivity = tf.compat.v1.get_variable(
                "input_connectivity",
                [N_rec, N_in],
                initializer=self.initializer.get("input_connectivity"),
                trainable=False,
            )
            self.rec_connectivity = tf.compat.v1.get_variable(
                "rec_connectivity",
                [N_rec, N_rec],
                initializer=self.initializer.get("rec_connectivity"),
                trainable=False,
            )
            self.output_connectivity = tf.compat.v1.get_variable(
                "output_connectivity",
                [N_out, N_rec],
    ...
    def train(self, trial_batch_generator, train_params={}, is_colab=False):
        ...
        if (
            performance_cutoff is not None and performance_measure is None
        ) or (performance_cutoff is None and performance_measure is not None):
            raise UserWarning(
        ...
        with train_progress:
            task_id = train_progress.add_task(
                "Training",
                start=True,
                total=int(training_iters / batch_size),
                loss=reg_loss,
    ...
    def test(self, trial_batch):
        ...
        # --------------------------------------------------
        outputs, states = self.sess.run(
            [self.predictions, self.states], feed_dict={self.x: trial_batch}
        )
