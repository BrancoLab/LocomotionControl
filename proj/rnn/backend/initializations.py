...
class WeightInitializer(object):
    ...
    def __init__(self, **kwargs):
        ...
        if self.load_weights_path is not None:
            ...
            # ----------------------------------
            self.initializations = dict(
                np.load(self.load_weights_path, allow_pickle=True)
            )
            if type(self.initializations["dale_ratio"]) == np.ndarray:
                self.initializations["dale_ratio"] = self.initializations[
                    "dale_ratio"
                ].item()

        else:
            if (
                kwargs.get("W_rec", None) is None
                and type(self).__name__ == "WeightInitializer"
            ):
                warn(
                    "This network may not train since the eigenvalues of W_rec are not regulated in any way."
                )
            ...
            self.rand_W_rec_init = self.get_rand_init_func(
                kwargs.get("which_rand_W_rec_init", self.rand_init)
            )
            self.rand_init = self.get_rand_init_func(self.rand_init)
            if (
                self.rand_init == self.const_unif_init
                or self.rand_W_rec_init == self.const_unif_init
            ):
                self.init_minval = kwargs.get("init_minval", -0.1)
                self.init_maxval = kwargs.get("init_maxval", 0.1)
            ...
            self.initializations["input_connectivity"] = kwargs.get(
                "input_connectivity", np.ones([N_rec, N_in])
            )
            assert self.initializations["input_connectivity"].shape == (
                N_rec,
                N_in,
            )
            self.initializations["rec_connectivity"] = kwargs.get(
                "rec_connectivity", np.ones([N_rec, N_rec])
            )
            assert self.initializations["rec_connectivity"].shape == (
                N_rec,
                N_rec,
            )
            self.initializations["output_connectivity"] = kwargs.get(
                "output_connectivity", np.ones([N_out, N_rec])
            )
            assert self.initializations["output_connectivity"].shape == (
                N_out,
                N_rec,
            )
            ...
            if not self.autapses:
                self.initializations["rec_connectivity"][
                    np.eye(N_rec) == 1
                ] = 0

            # Dale's constraint
            self.initializations["dale_ratio"] = dale_ratio = kwargs.get(
                "dale_ratio", None
            )
            if type(self.initializations["dale_ratio"]) == np.ndarray:
                self.initializations[
                    "dale_ratio"
                ] = dale_ratio = self.initializations["dale_ratio"].item()
            if dale_ratio is not None and (dale_ratio < 0 or dale_ratio > 1):
                print(
                    "Need 0 <= dale_ratio <= 1. dale_ratio was: "
                    + str(dale_ratio)
                )
                raise
            ...
            self.initializations["W_in"] = kwargs.get(
                "W_in",
                self.rand_init(self.initializations["input_connectivity"]),
            )
            assert self.initializations["W_in"].shape == (N_rec, N_in)
            self.initializations["W_out"] = kwargs.get(
                "W_out",
                self.rand_init(self.initializations["output_connectivity"]),
            )
            assert self.initializations["W_out"].shape == (N_out, N_rec)
            self.initializations["W_rec"] = kwargs.get(
                "W_rec",
                self.rand_W_rec_init(self.initializations["rec_connectivity"]),
            )
            assert self.initializations["W_rec"].shape == (N_rec, N_rec)

            self.initializations["b_rec"] = kwargs.get(
                "b_rec", np.zeros(N_rec)
            )
            assert self.initializations["b_rec"].shape == (N_rec,)
            self.initializations["b_out"] = kwargs.get(
                "b_out", np.zeros(N_out)
            )
            assert self.initializations["b_out"].shape == (N_out,)

            self.initializations["init_state"] = kwargs.get(
                "init_state", 0.1 + 0.01 * np.random.randn(N_rec)
            )
    ...
    def const_unif_init(self, connectivity):
        ...
        return (maxval - minval) * np.random.rand(
    ...
    def get(self, tensor_name):
        ...
        return tf.compat.v1.constant_initializer(
            self.initializations[tensor_name]
        )
    ...
    def balance_dale_ratio(self):
        ...
        if dale_ratio is not None:
            ...
            dale_vec[int(dale_ratio * self.N_rec) :] = dale_ratio / (
                1 - dale_ratio
            )
            dale_rec = (
                np.diag(dale_vec)
            ...
            self.initializations["W_rec"] = np.matmul(
                self.initializations["W_rec"], dale_rec
            )
        return
...
class GaussianSpectralRadius(WeightInitializer):
    ...
    def __init__(self, **kwargs):
        ...
        self.initializations["W_rec"] = (
            self.spec_rad
            * self.initializations["W_rec"]
            / np.max(np.abs(np.linalg.eig(self.initializations["W_rec"])[0]))
        )
