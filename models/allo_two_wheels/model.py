...
class Model(Model):
    ...
    def predict_next_state(self, curr_x, u):
        ...
        dtheta = torque / self.m
        dv = f_tot / (self.m * (np.abs(torque) + self.alpha))
...
# ------------------------------ MODEL GRADIENTS ----------------------------- #
    ...
    def calc_f_x(xs, us, dt):
        ...
        f_x[:, 0, 2] = -np.sin(theta) * v
        f_x[:, 0, 3] = np.cos(theta)
        f_x[:, 1, 2] = np.cos(theta) * v
        f_x[:, 1, 3] = np.sin(theta)
    ...
    def calc_f_u(self, xs, us, dt):
        ...
        # dv/du
        aval = np.abs(us[:, 0] - us[:, 1])
        denum = self.m * aval * (aval + self.alpha)**2
        denum[denum == 0] = .001

        f_u[:, 3, 0] = (2*us[:, 1]**2 - 2*us[:, 0]*us[:, 1] + self.alpha*aval)/denum
        f_u[:, 3, 1] = (2*us[:, 0]**2 - 2*us[:, 0]*us[:, 1] + self.alpha*aval)/denum
