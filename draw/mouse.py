...
from myterial import grey_darker, grey_dark,red_dark, purple, purple_darker, indigo, indigo_darker
...
class Mouse:
    def __init__(self, tracking:dict, at_frame:int=0, show_paws:bool=True,  ax:plt.Axes = None):
        '''
    ...
    def plot_body_line(self, tracking:dict, at_frame:int, ax:plt.Axes):
        ...
        for bp1, bp2 in bps:
            ax.plot(
                ...
                lw=4, color=grey_darker, zorder=2
            )
        Dot(tracking['body']['x'][at_frame], tracking['body']['y'][at_frame], s=150, color='k')
    ...
    def plot_head(self, tracking:dict, at_frame:int, ax:plt.Axes):
        ...
        # draw an arrow
        Arrow(tracking['neck']['x'][at_frame], tracking['neck']['y'][at_frame], head_vec.angle, L=2, color=red_dark, zorder=3)

    def plot_paws(self, tracking, at_frame, ax):
        paws = ('left_fl', 'right_fl', 'right_hl', 'left_hl')
        for paw in paws:
            Dot(tracking[paw]['x'][at_frame], tracking[paw]['y'][at_frame], s=50, color=COLORS[paw])
...
class Mice:
    def __init__(self, tracking:dict, step:int=10, show_paws:bool=True,  ax:plt.Axes = None):
