...
class RLWorld:
    ...
    def get_reward(self, agent, state_idx):
        """
        ...
        # if  reward > 0:
        #     raise ValueError('Should not have positive reward')

        # if np.isnan(reward):
        #     self.progress.log('WORLD', 'Nans in reward !!!')
        #     return None
        # return dx, dy, reward

        reward = self.curr_traj_waypoint_idx / len(self.trajectory)

        if reward > 5:
            raise ValueError("Very large reward!")
        return dx, dy, reward
