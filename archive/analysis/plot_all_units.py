...
for session in track(sessions):
    ...
    recording = (db_tables.Recording & f'name="{session}"').fetch(
        as_dict=True
    )[0]
    ...
    for i, unit in units.iterrows():
        ...
        save_name = (
            f"{unit.brain_region}_{unit.mouse_id}_{unit.name}_{unit.unit_id}"
        )
...
draw.Tracking.heatmap(
    unit_tracking.x, unit_tracking.y, ax=axes["tracking"], bins="log"
)
