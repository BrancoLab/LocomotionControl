...
save_folder = Path(
    r"D:\Dropbox (UCL)\Rotation_vte\Locomotion\analysis\bouts_clips"
)
...
for session in track(sessions):
    ...
    save_path = save_folder / (
        session + f"_{len(session_bouts)}_bouts_clips_{FPS}fps.mp4"
    )
    ...
    for i, bout in session_bouts.iterrows():
        ...
        for frame in range(bout.start_frame - 10, bout.end_frame + 10):
            ...
            if not ret:
                raise ValueError(
                    f"Failed {video} to open video at frame {frame}"
                )
