...
class Inspector:
    ...
    def fetch(self):
        ...
        self.rsites = pd.DataFrame(
            (
                Probe.RecordingSite & recording & f'probe_configuration="{cf}"'
            ).fetch()
        )
    ...
    def plot(
        ...
        logger.warning(
            "Inspector should plot firing rate vs heading direction and heading ang vel not ang vel"
        )
...
if __name__ == "__main__":
    for rec in Recording().fetch("name"):
        ...
        # try:
        insp = Inspector(rec, firing_rate_window=33, events_window_s=5,)
