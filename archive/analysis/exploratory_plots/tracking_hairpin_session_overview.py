...
def plot_hairpin_tracking(
    ...
    # draw tracking 1D
    visuals.plot_tracking_linearized(
        downsampled_tracking, ax=axes["B"], plot=True, color=[0.6, 0.6, 0.6]
    )
    ...
    visuals.plot_aligned(
        tracking.speed, bouts.start_frame, axes["C"], "after", alpha=0.5
    )
    visuals.plot_aligned(
        tracking.speed, bouts.end_frame, axes["D"], "pre", alpha=0.5
    )
    ...
    axes["E"].set(
        ylabel="counts", xlabel="duration (s)", title="Bouts duration"
    )
    ...
    axes["R"].set(
        xlabel="speed (cm/s)", ylabel="ang vel (deg/s)", title="moving"
    )
    ...
    axes["W"].set(
        xlabel="speed (cm/s)", ylabel="ang vel (deg/s)", title="bouts heatmap"
    )

    for ax in "AFGLMPQ":
        axes[ax].axis("equal")
        axes[ax].set(
            xlim=[-5, 45], ylim=[-5, 65], xticks=[0, 40], yticks=[0, 60]
        )
