...
(velocity, acceleration_vec, tangent,) = smooth_path_vectors(
    path, window=window
)
path = Path(tracking.x[window:], tracking.y[window:])
...
draw.Tracking.heatmap(
    unit_path.x, unit_path.y, bins="log", mincnt=3, gridsize=30, ax=axes["A"]
)

for ax, var in zip("BCDE", ("speed", "acceleration_mag", "theta", "thetadot")):
    axes[ax].scatter(
        path[var], unit.firing_rate[window:], alpha=0.25, color="k"
    )
    axes[ax].set(title=var)

for ax, var in zip("bcde", ("speed", "acceleration_mag", "theta", "thetadot")):
    draw.Hist(path[var], ax=axes[ax], density=True, alpha=0.25, color="k")
