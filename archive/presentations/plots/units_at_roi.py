...
for rn, (region, uns) in enumerate(_units.items()):
    ...
    for n, unit in uns.iterrows():
        ...
        draw.Raster(
            unit.spikes, starts, ends, ax=axes[n, rn], color=unit.color, lw=5
        )
...
for bout in bouts:
    ...
    axes["B"].plot(bout.velocity.magnitude, color="k", lw=0.5)
    axes["C"].plot(bout.thetadot, color="k", lw=0.5)
...
for n, bout in enumerate(registered_bouts):
    ...
    axes["B"].plot(bout.speed, lw=0.5, color="k")
    axes["C"].plot(bout.acceleration_mag, lw=0.5, color="k")
    axes["D"].plot(resampled_frates[n], lw=0.5, color="k")


axes["B"].plot(
    np.mean(np.vstack([b.speed for b in registered_bouts]), 0), lw=3, color="r"
)
axes["C"].plot(
    np.mean(np.vstack([b.acceleration_mag for b in registered_bouts]), 0),
    lw=3,
    color="r",
)
