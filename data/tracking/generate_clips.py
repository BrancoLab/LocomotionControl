...
for video in track(raw_videos):
    try:
        ...
        logger.warning(f'Failed to get data bout video {video.name} with error: {e}')
        continue

    clips_starts = np.random.uniform(2000, n_frames-2000, size=N_clips_per_vid).astype(np.int32)
    for n, start_frame in enumerate(clips_starts):
        ...
        video_utils.trim_clip(video, save_path, start_frame=start_frame, end_frame=start_frame+N_frames_per_vid, fps=60)
