...
def plot_speeds(body_speed, start:int=0, end:int=-1, is_moving:np.ndarray=None, ax:plt.axis=None, show:bool=True,  **other_speeds):
    '''
...
def plot_bout_start_end(body_speed, start:int=0, end:int=-1, is_moving:np.ndarray=None, precise_end:int=None, precise_start:int=None, speed_th:float=1,**other_speeds):
    '''
    ...
    # plot start & end
    plot_speeds(body_speed, start=start-60 if start>60 else 0, end=start+60, ax=axes[0], show=False,  **other_speeds)
    plot_speeds(body_speed, start=end-60, end=end+60, ax=axes[1], show=False,  **other_speeds)

    # mark precise starts and ends
    if precise_start is not None:
        axes[0].axvline(60 + (precise_start - start), lw=4, alpha=.5, color='salmon')
    if precise_end is not None:
        axes[1].axvline(60 + (precise_end - end), lw=4, alpha=.5, color='salmon')

    for name, ax in zip(('start', 'end'), axes):
        ax.axvline(60, ls='--', color=[.4, .4, .4], zorder=-1)
        ax.axhline(speed_th, color=[.4, .4, .4], zorder=-1)
        ax.set(title=name)
    plt.show()


def get_when_moving(body_speed:np.ndarray, speed_th:float, max_pause:float, min_duration:float, min_peak_speed:float) -> np.ndarray:
    '''
    ...
    # remove pauses
    onsets, offsets = get_onset_offset(is_moving, .5)
    ...
    for onset, offset in zip(onsets, offsets):
        reached_speed = len(np.where(body_speed[onset:offset] >= min_peak_speed)[0]) > 0
...
def get_threshold_crossing(tracking:tuple, frame:int, direction:str, threshold:float)->int:
    '''
...
def get_bout_start_end_times(tracking:tuple, bstart:int, bend:int, speed_th:float) -> Tuple[int, int]:
    '''
...
def check_gcoord_delta(tracking, bstart:int, bend:int, min_gcoord_delta:float) -> bool:
    gstart, gend = tracking.body.global_coord[bstart], tracking.body.global_coord[bend]
...
def get_bout_complete_and_rois(tracking:tuple, bstart:int, bend:int) -> Tuple[str, int, int]:
    # check if it was a complete trip
    gstart, gend = tracking.body.global_coord[bstart], tracking.body.global_coord[bend]
    complete = abs(gend - gstart) > .8
...
def get_session_bouts(key:dict, tracking:pd.DataFrame, is_hairpin:bool, speed_th:float, max_pause:float, min_duration:float, min_peak_speed:float, min_gcoord_delta:float) -> list:
    '''
    ...
    # get when the mouse is moving
    is_moving = get_when_moving(tracking.body.speed, speed_th, max_pause, min_duration, min_peak_speed)
    ...
    for bstart, bend in zip(onsets, offsets):
        ...
        bstart, bend = get_bout_start_end_times(tracking, bstart, bend, speed_th)
        ...
        if is_hairpin:
            ...
            if not check_gcoord_delta(tracking, bstart, bend, min_gcoord_delta):
                continue
            ...
            complete, start_roi, end_roi = get_bout_complete_and_rois(tracking, bstart, bend)
        else:
    ...
    logger.debug(f' kept {len(bouts)} bouts of which {n_complete} are complete')
    return bouts