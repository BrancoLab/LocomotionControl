...
def select_rows(data:Union[pd.DataFrame, np.array], skip:int=200, pitch:int=20, N:int=384):
...
def place_probe_recording_sites(probe_metadata:dict, n_sites:int = 384) -> list:
    ...
    if not probe_metadata['implanted_depth'] * 1000 == len(points):
        logger.warning('The reconstructed probe should have one point for every um of inserted probe')
        return
    ...
    regions_metadata = pd.read_csv(
        Path(probe_metadata['reconstructed_track_filepath']).with_suffix('.csv')
    ).iloc[::-1]
    ...
    for n in range(n_sites):
        if n >= len(electrodes):
            ...
            {
                'site_id':ids[n], 
                'registered_brain_coordinates': np.full(3, fill_value=np.nan), 
                'brain_region':'OUT', 
                'brain_region_id':-1,
            ...
            try:
                ...
                color_rgb = [c/255 for c in atlas._get_from_structure(acro, 'rgb_triplet')]
            ...
            recording_sites.append(
                {'site_id':ids[n], 'registered_brain_coordinates': electrodes[n], 
                        'brain_region':acro, 'brain_region_id':rid, 'probe_coordinates':int(n * 20), 'color':color}
            )
