...
from myterial import salmon_dark, orange, amber, green, teal, cyan_dark, blue, indigo, purple, pink, blue_grey, brown, black
...
class Segment:
    ...
    def interpolate(self, n_points:int):
        ...
        self.ids = np.ones(len(self.line)) * self.iid  # assign at each point the segment's ID
        return self.line

    def draw(self, ax: plt.axis):
        ax.scatter(*self.p0, s=200, lw=1, ec=[.3, .3, .3], color=self.color, zorder=100)
        if self.last:
            ax.scatter(*self.p1, s=200, lw=1, ec=[.3, .3, .3], color=self.color, zorder=100)

        ax.plot([self.p0[0], self.p1[0]], [self.p0[1], self.p1[1]], lw=6, color=[.3, .3, .3], zorder=98)
        ax.plot([self.p0[0], self.p1[0]], [self.p0[1], self.p1[1]], lw=5, color=self.color, zorder=99)
...
class HairpinTrace:
    ...
    def assign_tracking(self, x:np.ndarray, y:np.ndarray):
        ...
        idxs[(global_coordinates > .02)&(global_coordinates < .11)] = 1
        idxs[(global_coordinates > .11)&(global_coordinates < .17)] = 2
        idxs[(global_coordinates > .17)&(global_coordinates < .28)] = 3
        idxs[(global_coordinates > .28)&(global_coordinates < .36)] = 4
        idxs[(global_coordinates > .36)&(global_coordinates < .48)] = 5
        idxs[(global_coordinates > .48)&(global_coordinates < .53)] = 6
        idxs[(global_coordinates > .53)&(global_coordinates < .66)] = 7
        idxs[(global_coordinates > .66)&(global_coordinates < .72)] = 8
        idxs[(global_coordinates > .72)&(global_coordinates < .78)] = 9
        idxs[(global_coordinates > .78)&(global_coordinates < .85)] = 10
        idxs[(global_coordinates > .85)&(global_coordinates < .98)] = 11
        idxs[global_coordinates > .98] = 12
    ...
    def build(self):
        ...
        samples_per_segment = [int(seg.length * samples_per_unit_length) for seg in self.segments]
        ...
        self.trace_length = np.sum([len(segment.line) for segment in self.segments])

        # stach all segmnts into a curve
        self.trace = np.vstack([segment.line for segment in self.segments])
        self.trace_ids = np.concatenate([segment.ids for segment in self.segments])


    def draw(self, ax: plt.axis = None, tracking:dict=None, colorby:str='segment'):
        ...
        if tracking is not None:
            if colorby == 'segment':
                ...
                c = self.colors_from_global_coordinates(tracking['global_coord'])
            ax.scatter(tracking['x'][::5], tracking['y'][::5], c=c[::5], s=50, alpha=.8)
