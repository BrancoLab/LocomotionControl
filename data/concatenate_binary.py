...
def concatenate(source1: Path, source2: Path, dest: Path): 
    ...
    logger.info(f'About to merge: "{source1.name}" ({size(source1)}) and "{source2.name}" ({size(source2)})')
    ...
    for source in (source1, source2):
        ...
        tid = progress.add_task(f'Processing "{source.name}"', total=size(source, fmt=False))
        ...
        while True:
            ...
            if len(chunk) < chunk_size:
                raise ValueError(f'Got chunk of length: {len(chunk)}, expected {chunk_size} bytes')
...
if __name__ == '__main__':
    ...
    s1 = recs_fld / Path(r1) / Path(r1+'_imec0') / Path(r1+'_t0.imec0.ap.bin')
    s2 = recs_fld / Path(r2) / Path(r2+'_imec0') / Path(r2+'_t0.imec0.ap.bin')
    dest = recs_fld / Path(d) / Path(d+'_imec0') / Path(d+'_t0.imec0.ap.bin')
