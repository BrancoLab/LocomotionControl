...
from fcutils.maths.geometry import (
    calc_distance_between_points_two_vectors_2d as get_bone_length,
)
...
def compute_body_segments(tracking, segments, smooth_orientation=True):
    ...
    for bone, (bp1, bp2) in segments.items():
        ...
        if not smooth_orientation:
            ...
            bone_orientation = median_filter_1d(
                get_bone_angle(
                    bp1.x.values, bp1.y.values, bp2.x.values, bp2.y.values,
                ),
                kernel=41,
        ...
        bp1_tr, bp2_tr = (
            np.array([bp1.x.values, bp1.y.values]).T,
            np.array([bp2.x.values, bp2.y.values]).T,
        )
        nan_idxs = (
        ...
        bone_length = get_bone_length(
            np.nan_to_num(bp1_tr), np.nan_to_num(bp1_tr)
        )
